...
osThreadId ToggleGreenTaskHandle;
osThreadId ToggleRedTaskHandle;
/* USER CODE BEGIN PV */
...
void StartToggleGreenTask(void const * argument);
void StartToggleRedTask(void const * argument);
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* definition and creation of ToggleGreenTask */
  osThreadDef(ToggleGreenTask, StartToggleGreenTask, osPriorityNormal, 0, 128);
  ToggleGreenTaskHandle = osThreadCreate(osThread(ToggleGreenTask), NULL);

  /* definition and creation of ToggleRedTask */
  osThreadDef(ToggleRedTask, StartToggleRedTask, osPriorityRealtime, 0, 128);
  ToggleRedTaskHandle = osThreadCreate(osThread(ToggleRedTask), NULL);
...
/* USER CODE BEGIN Header_StartToggleGreenTask */
/**
  * @brief  Function implementing the ToggleGreenTask thread.
  * @param  argument: Not used 
  * @retval None
  */
/* USER CODE END Header_StartToggleGreenTask */
void StartToggleGreenTask(void const * argument)
{
	TickType_t TaskTimeStamp, last;
	...
	for(;;){
		...
		HAL_GPIO_WritePin(EGreen_GPIO_Port, EGreen_Pin, GPIO_PIN_RESET); // Two seconds on
...
/* USER CODE BEGIN Header_StartToggleRedTask */
/**
* @brief Function implementing the ToggleRedTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartToggleRedTask */
void StartToggleRedTask(void const * argument)
{
	TickType_t TaskTimeStamp, last;
	...
	for(;;){
		...
		HAL_GPIO_WritePin(ERed_GPIO_Port, ERed_Pin, GPIO_PIN_RESET); // Two seconds on
		HAL_GPIO_WritePin(EYellow_GPIO_Port, EYellow_Pin, GPIO_PIN_RESET);
		osDelayUntil(&TaskTimeStamp, DelayTimeMsec);
	}
}
