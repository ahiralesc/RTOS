...
void toggleBlueHook(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(toggleBlue, toggleBlueHook, osPriorityNormal, 0, 128);
...
void toggleGreenHook(void const * argument)
{
	...
	toggle(EGreen_GPIO_Port, EGreen_Pin, (uint32_t) 100, 3000);
	event_flag = event_flag ^ 1;
	toggle(EGreen_GPIO_Port, EGreen_Pin, (uint32_t) 100, 2000);
	HAL_GPIO_WritePin(EGreen_GPIO_Port, EGreen_Pin, GPIO_PIN_RESET);
	vTaskSuspend(NULL);
...
void toggleRedHook(void const * argument)
{
	...
	event_flag = event_flag | 2;
	toggle(ERed_GPIO_Port, ERed_Pin, (uint32_t) 1000, 4000);
	event_flag = event_flag ^ 2;
	toggle(ERed_GPIO_Port, ERed_Pin, (uint32_t) 100, 2000);
